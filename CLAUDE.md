...
- **API キー・トークン・パスワード・接続文字列など、漏洩の可能性がある情報は絶対にコミットしない。**
- `.env` / `.env.local` などの秘匿ファイルは `.gitignore` で除外する。コミット前に `git diff --cached --name-only` でステージ内容を確認し、秘匿ファイルが含まれていないことを確かめる。
- シークレットはコード（ソース・設定・テスト）に直書きせず、必ず環境変数から読む。サンプルは `.env.example` にキー名のみ記載し、実値は書かない。
- 不安がある場合は、コミット前に `/security-review` で変更内容のセキュリティチェックを行う。
- 万一シークレットをコミット・プッシュしてしまった場合は、該当キーを即座に無効化（ローテーション）し、履歴からの除去を検討する。
