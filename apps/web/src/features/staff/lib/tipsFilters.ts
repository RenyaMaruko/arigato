/**
 * 受取履歴（/staff/history）の店舗・期間フィルタの純粋ロジック。
 * 期間プリセット（すべて／今月／先月／今年）から API に渡す from/to（ISO 文字列）を計算する。
 * 計算は純粋関数に切り出し（now を引数で受ける）、UI から独立して単体テストできるようにする。
 */

// 期間プリセット。all は期間フィルタ無し。
export type TipsPeriod = "all" | "thisMonth" | "lastMonth" | "thisYear";

// 期間プリセットの一覧（セレクタの並び順に合わせる）
export const TIPS_PERIODS: TipsPeriod[] = ["all", "thisMonth", "lastMonth", "thisYear"];

// 期間から導く受取日時の範囲。from は含む（>=）、to は排他（<）。
// 未指定（フィルタ無し）の端は undefined。
export type TipsPeriodRange = {
  from?: string;
  to?: string;
};
 ...
 */
export function computePeriodRange(period: TipsPeriod, now: Date = new Date()): TipsPeriodRange {
  // すべて＝期間フィルタ無し
  if (period === "all") {
    return {};
  }
