...
/**
 * 孤児 pending の掃除（期限切れ判定）の純粋関数テスト。
 * 突合ジョブが「作成から24時間超の pending tip を failed 化してよいか」を判定する契約を検証する。
 */
describe("isPendingTipExpired（pending の期限切れ判定）", () => {
  const now = new Date("2026-07-06T12:00:00Z");

  it("作成から24時間超なら期限切れ（掃除対象）", () => {
    const createdAt = new Date(now.getTime() - (PENDING_TIP_EXPIRY_HOURS + 1) * 60 * 60 * 1000);
    expect(isPendingTipExpired(createdAt, now)).toBe(true);
  });

  it("作成から24時間未満なら期限内（掃除しない）", () => {
    const createdAt = new Date(now.getTime() - 1 * 60 * 60 * 1000);
    expect(isPendingTipExpired(createdAt, now)).toBe(false);
  });

  it("ちょうど24時間は期限内（超えたときだけ掃除する）", () => {
    const createdAt = new Date(now.getTime() - PENDING_TIP_EXPIRY_HOURS * 60 * 60 * 1000);
    expect(isPendingTipExpired(createdAt, now)).toBe(false);
  });
});

describe("shouldExpirePendingIntentStatus（期限切れで failed 化してよい PI 状態か）", () => {
  it("お客さまの操作待ち（requires_*）は掃除対象", () => {
    expect(shouldExpirePendingIntentStatus("requires_payment_method")).toBe(true);
    expect(shouldExpirePendingIntentStatus("requires_confirmation")).toBe(true);
    expect(shouldExpirePendingIntentStatus("requires_action")).toBe(true);
  });

  it("processing（資金が動いている可能性）と確定系は掃除対象にしない", () => {
    expect(shouldExpirePendingIntentStatus("processing")).toBe(false);
    expect(shouldExpirePendingIntentStatus("succeeded")).toBe(false);
    expect(shouldExpirePendingIntentStatus("canceled")).toBe(false);
  });
});
