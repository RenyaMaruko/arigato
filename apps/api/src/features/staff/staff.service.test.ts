...
import { TutorialKeySchema } from "@arigato/shared";
import type {
  StaffRepository,
  InviteRow,
  StaffProfileRow,
...
/**
 * チュートリアル既読（user_tutorial・アカウント紐づけ）のテスト。
 * 検証する契約:
 * - GET /staff/me（getStaffMe）の応答に seenTutorials（既読キー一覧）が同乗する
 * - markTutorialSeen は冪等（二度呼んでも成功・行は増えない）
 * - 本人スコープ（自分の auth_user_id にのみ書き、他人の既読状態に影響しない）
 * - キーのホワイトリスト検証（TutorialKeySchema。未知キーは Route の zValidator が 400 で弾く）
 */
describe("staff.service チュートリアル既読（user_tutorial）", () => {
  let mock: ReturnType<typeof createMockRepo>;

  beforeEach(async () => {
    mock = createMockRepo();
    // 本人・他人のプロフィールを用意する（既読はアカウント紐づけだが、me 応答の検証に使う）
    await createStaffProfile(mock.repo, buildUrl, makeCreateConnectedAccount(), "auth-user-1", {
      displayName: "山田 さくら",
    });
    await createStaffProfile(mock.repo, buildUrl, makeCreateConnectedAccount(), "auth-user-2", {
  ...
  it("getStaffMe: 未既読なら seenTutorials は空配列（me 応答に同乗する）", async () => {
    const me = await getStaffMe(mock.repo, buildUrl, "auth-user-1");
    expect(me!.seenTutorials).toEqual([]);
  });

  it("markTutorialSeen: 既読にすると getStaffMe の seenTutorials に載る", async () => {
    const result = await markTutorialSeen(mock.repo, "auth-user-1", "welcome");
    expect(result).toEqual({ key: "welcome" });
    const me = await getStaffMe(mock.repo, buildUrl, "auth-user-1");
    expect(me!.seenTutorials).toContain("welcome");
    // 別キー（mode_switch）はまだ未既読
    expect(me!.seenTutorials).not.toContain("mode_switch");
  });

  it("markTutorialSeen: 冪等（同じキーを二度既読にしても成功し、行は増えない）", async () => {
    await markTutorialSeen(mock.repo, "auth-user-1", "mode_switch");
    // 2回目も例外なく成功扱い
    const again = await markTutorialSeen(mock.repo, "auth-user-1", "mode_switch");
    expect(again).toEqual({ key: "mode_switch" });
    // 記録は1件のまま（重複行を作らない）
    const me = await getStaffMe(mock.repo, buildUrl, "auth-user-1");
    expect(me!.seenTutorials).toEqual(["mode_switch"]);
  });

  it("markTutorialSeen: 本人スコープ（他人の seenTutorials には影響しない）", async () => {
    await markTutorialSeen(mock.repo, "auth-user-1", "welcome");
    // 本人には載り、他人には載らない
    const me1 = await getStaffMe(mock.repo, buildUrl, "auth-user-1");
    const me2 = await getStaffMe(mock.repo, buildUrl, "auth-user-2");
    expect(me1!.seenTutorials).toEqual(["welcome"]);
    expect(me2!.seenTutorials).toEqual([]);
  });

  it("markTutorialSeen: 複数キーの既読を両方保持する（welcome と mode_switch）", async () => {
    await markTutorialSeen(mock.repo, "auth-user-1", "welcome");
    await markTutorialSeen(mock.repo, "auth-user-1", "mode_switch");
    const me = await getStaffMe(mock.repo, buildUrl, "auth-user-1");
    expect(me!.seenTutorials).toHaveLength(2);
    expect(me!.seenTutorials).toContain("welcome");
    expect(me!.seenTutorials).toContain("mode_switch");
  });

  it("TutorialKeySchema: ホワイトリスト検証（既知キーは通り、未知キーは弾く＝Route で 400）", () => {
    // 既知キーは通る
    expect(TutorialKeySchema.safeParse("welcome").success).toBe(true);
    expect(TutorialKeySchema.safeParse("mode_switch").success).toBe(true);
    // 未知キー・空・別型は弾く（ゴミデータの書き込み防止）
    expect(TutorialKeySchema.safeParse("unknown_key").success).toBe(false);
    expect(TutorialKeySchema.safeParse("").success).toBe(false);
    expect(TutorialKeySchema.safeParse(123).success).toBe(false);
  });
});
